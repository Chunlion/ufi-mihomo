...
TProxy 或关闭接管时，运行配置移除 `tun` 段，原参数保存在同一文件的 `x-kano-tun` 中；切回 TUN 时自动恢复，自定义协议栈、MTU 等参数继续保留。请保留该字段以便恢复停用参数。DNS、IPv6 和 TUN 路由仍按 F50 接管要求适配，启用私网定向代理后会叠加对应规则；不支持完全原样运行或保留 YAML 注释、排版。
