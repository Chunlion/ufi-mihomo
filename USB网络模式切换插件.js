...
`;
  const backendValid = async () => {
    ...
    return res.code === 0 && res.text.indexOf('SELFTEST=KUNM_2.4.5_OK') >= 0;
  };
...
` + checksumCmd(stage) + `"$SH" -n ${sq(stage)} && "$SH" ${sq(stage)} selftest`);
      ...
      if (!verify.text.includes('SELFTEST=KUNM_2.4.5_OK')) throw new Error('\u672a\u53d6\u5f97\u81ea\u68c0\u901a\u8fc7\u6807\u8bb0');
      step = '停止旧守护并等待任务退出';
      await quiesce();
      step = '替换后台文件';
