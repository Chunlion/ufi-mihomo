...
${script}`,
  ...
  const removeBootLinesCmd = () => `
        if [ -f ${shellQuote(BOOT_FILE)} ]; then
          sed -i '/\/data\/clash\/Scripts\/Clash.Service start/d' ${shellQuote(BOOT_FILE)}
          sed -i '/Clash.Inotify/d' ${shellQuote(BOOT_FILE)}
          sed -i '/Clash.MacBypass/d' ${shellQuote(BOOT_FILE)}
          sed -i '/Clash.PolicyTools/d' ${shellQuote(BOOT_FILE)}
        fi
        `;
...
KANO_YQ_SMOKE_EOF
  ...
  const buildSubUrlsFileText = (sources = [], mode = SUB_RULE_MODE_TEMPLATE) => {
  ...
  const persistSubSourcesForTemplate = async (sources = [], mode = SUB_RULE_MODE_TEMPLATE) => {
    const storedSources = normalizeStoredSubSourceList(sources);
    if (storedSources.length == 0 || normalizeSubSourceList(storedSources).length == 0) return false;
    const subUrlsText = buildSubUrlsFileText(storedSources, mode);
    const res = await runShellWithRoot(`
        set -e
        ...
        KANO_CONVERT_LINE="$(grep -m 1 '^# KANO_SUB_CONVERT_MODE=' "$SUB" 2>/dev/null | tr -d '\r')"
        printf '%s' ${shellQuote(subUrlsText)} > "$SUB_NEW"
        if [ -n "$KANO_CONVERT_LINE" ]; then
          { sed -n '1p' "$SUB_NEW"; printf '%s\\n' "$KANO_CONVERT_LINE"; sed -n '2,$p' "$SUB_NEW"; } > "$SUB_NEW.merged"
          mv -f "$SUB_NEW.merged" "$SUB_NEW"
        fi
        chmod 600 "$SUB_NEW"
        mv -f "$SUB_NEW" "$SUB"
        trap - EXIT
  ...
  const buildManagedProxyProviders = (sources = [], { emptyUrls = false, ensureOne = false } = {}) => {
  ...
  const writeRuntimeConfigFromTemplate = async (sources = [], { backup = true, showToast = false, forceTemplate = false } = {}) => {
    const cleanSources = normalizeSubSourceList(sources);
...
KANO_WRITE_CHECK_EOF
  ...
  const readControllerInfo = async () => {
    ...
    return {
      ...
      configSource: externalController && secret ? 'real' : 'fallback',
    };
  };
  ...
  const buildApiCurl = (
    ...
    return [
      ...
      '-H',
      shellQuote(`Authorization: Bearer ${info.secret || '123456'}`),
      typeArg,
      bodyArg,
      shellQuote(url),
    ].filter(Boolean).join(' ');
  ...
  const callMihomoApi = async (
    ...
    const responseText = raw.replace(/\n?__HTTP_CODE:\d{3}__\s*$/, '').trim();
    return {
      ...res,
  ...
  const flushMihomoRuntimeCaches = async () => {
    ...
    return targets.map((target, index) => ({
      ...
      message: sanitizeSubscriptionSecrets(responses[index].responseText || responses[index].content || ''),
    }));
  };
  ...
  const classifyProviderUpdateError = (rawError = '', statusCode = 0) => {
    ...
    if (status == 401 || status == 403 || /(?:unauthorized|forbidden|token[^\n]*(?:invalid|expired))/.test(lower)) {
      return { type: 'permission', message: '订阅地址无权限或已失效', retryable: false };
    }
    ...
    if (!readiness.ok) {
      ...
      const results = requestedNames.map((name) => {
        ...
        const classified = providerMissing
          ...
          : { type: 'api_unavailable', message: 'Mihomo 控制 API 暂不可用' };
        return {
  ...
  const checkIsBootUp = async () => {
    const res = await runShellWithRoot(`
        if [ -f ${shellQuote(BOOT_FILE)} ] && grep -qxF ${shellQuote(BOOT_SERVICE_LINE)} ${shellQuote(BOOT_FILE)}; then
          echo 1
        else
          echo 0
        fi
  ...
  const checkIsInstalled = async () => {
    const res = await runShellWithRoot(`
        ...
        `);
    return res.success && String(res.content || '').trim() == '1';
  };
  ...
  const collectNetworkStatus = async () => {
    const res = await runShellWithRoot(`
        ...
        (df -k /data /tmp 2>/dev/null || df -k /data 2>/dev/null || true)
        echo
        echo "[yq runtime]"
  ...
  const startClashServiceClean = async ({ stopFirst = false, reason = '\u542f\u52a8' } = {}) =>
    runShellWithRoot(
      `
        ...
        echo "[restart] reason: ${shellQuote(reason)}"
        if [ ${shellQuote(stopFirst ? '1' : '0')} = '1' ]; then
          ${shellQuote(CLASH_SERVICE)} stop 2>&1 || true
          sleep 2
        fi
        ${flushGeneratedRulesCmd()}
        ${shellQuote(CLASH_SERVICE)} start
        rc=$?
        sleep 2
        exit "$rc"
        `,
  ...
  const waitForCoreApi = async (tries = 12, delayMs = 1000) => {
    ...
    for (let i = 0; i < tries; i++) {
      ...
      if (pid || i % 3 == 2 || i == tries - 1) {
        const version = await callMihomoApi('/version', 'GET', null, info, 2);
  ...
  const saveTemplate = async (file) => {
    ...
    try {
      ...
      if (templateSubSourcesToPersist.length > 0) {
        ...
        if (!(await persistSubSourcesForTemplate(templateSubSourcesToPersist, SUB_RULE_MODE_TEMPLATE))) {
  ...
  const showStatusDiagnostic = async () => {
    ...
    const versionPromise = Promise.all([controllerInfoPromise, pidPromise]).then(([info, pid]) =>
      pid ? callMihomoApi('/version', 'GET', null, info) : { success: false, statusCode: 0 });
    const connectionsPromise = Promise.all([controllerInfoPromise, pidPromise]).then(([info, pid]) =>
      pid ? callMihomoApi('/connections', 'GET', null, info, 5) : { success: false, statusCode: 0 });
    const [
      installed,
      bootUp,
      pid,
  ...
  const buildBootstrapConfig = (secret = '123456') => [
    'port: 7890',
    'socks-port: 7891',
    'mixed-port: 7892',
    'redir-port: 7893',
...
KANO_BOOTSTRAP_CONFIG
  ...
  btn_enabled.onclick = async () => {
    ...
    try {
      ...
      const logEl = el.querySelector('pre');
      while (true) {
        const dlog = await runShellWithRoot(
          `if [ -f ${shellQuote(DOWNLOAD_LOG)} ]; then timeout 2s awk '{print}' ${shellQuote(DOWNLOAD_LOG)}; fi`,
        );
        const lines = String(dlog.content || '').split('\n');
        log = lines.slice(-6).join('\n');
        setText(logEl, log || '\u7b49\u5f85\u4e0b\u8f7d...');
        ...
        if (log.includes('DOWNLOAD_FAILED')) {
          close();
          return createToast(`\u4e0b\u8f7d\u5931\u8d25\uff1a<br>${textToHtml(log)}`, 'red');
        }
      ...
      await runShellWithRoot(`rm -f ${shellQuote(DOWNLOAD_LOG)}`);
  ...
  const syncUnifiedDeviceBypassStorage = async () => {
    const res = await runShellWithRoot(`
        ...
        if [ -f "$BOOT" ]; then
          sed -i '/Clash.MacBypass/d' "$BOOT" 2>/dev/null || true
        fi
...
KANO_POLICY_TOOLS_EOF
  ...
  const refreshModeBadge = async () => {
    try {
      ...
      if (res.success) updateModeBadge(parsePolicyOptionsText(String(res.content || '')).traffic_mode);
    } catch (e) {
  ...
  const savePolicyState = async (state, { apply = true } = {}) => {
    ...
    const res = await runShellWithRoot(`
        ...
        if [ -f ${shellQuote(BOOT_FILE)} ]; then sed -i '/Clash.MacBypass/d' ${shellQuote(BOOT_FILE)} || true; fi
    ...
    const policyScriptReady = await ensurePolicyToolsScript({ syncStorage: false });
    if (!policyScriptReady) return false;
    if (await checkIsBootUp()) {
      await runShellWithRoot(addPolicyToolsBootLineCmd());
    }
    ...
    const { el, close } = createFixedToast(
      ...
      `
        <style>
          ...
          #kano_policy_shell .kp-footer{margin-top:12px;display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap;align-items:center;}
          #kano_policy_shell .kp-footer-right{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;}
  ...
  (async () => {
    ...
    style.textContent = `
      ...
      #IFRAME_KANO .kano-mm-badge{font-size:.56rem;padding:3px 8px;border-radius:999px;background:rgba(59,130,246,.16);border:1px solid rgba(147,197,253,.28);color:#bfdbfe;white-space:nowrap;}
      #IFRAME_KANO .kano-mm-box{padding:10px;border:1px solid rgba(148,163,184,.18);border-radius:16px;background:rgba(15,23,42,.36);}
      #IFRAME_KANO .kano-mm-status{font-size:.56rem;line-height:1.55;margin:0 0 10px;padding:7px 9px;border:1px solid rgba(96,165,250,.22);border-radius:12px;background:rgba(15,23,42,.46);color:#bfdbfe;}
      #mm_action_box{margin-bottom:10px;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;align-items:start;}
      #IFRAME_KANO .kano-action-group{border:1px solid rgba(148,163,184,.18);border-radius:14px;background:rgba(15,23,42,.44);overflow:hidden;}
      #IFRAME_KANO .kano-action-group>summary{cursor:pointer;list-style:none;padding:9px 11px;font-size:.62rem;font-weight:800;letter-spacing:.02em;color:#dbeafe;background:rgba(30,41,59,.55);display:flex;align-items:center;justify-content:space-between;gap:8px;}
      ...
      #IFRAME_KANO .kano-action-inner button{width:100%;min-width:0;}
      ...
      @media (max-width:720px){#mm_action_box{grid-template-columns:1fr;}#IFRAME_KANO #mm_iframe{min-height:360px;height:65vh;}}
...
`,
    ...
    const refreshPanel = async ({ forceShow = false } = {}) => {
      ...
      if (!isWebPanelVisible()) {
        createToast('Web \u9762\u677f\u5f53\u524d\u5df2\u9690\u85cf\uff0c\u70b9\u51fb\u201c\u663e\u793a Web \u9762\u677f\u201d\u540e\u518d\u5237\u65b0\u3002', 'yellow', 4500);
        return;
      }
      const iframe = document.querySelector('#mm_iframe');
    ...
    controllerSettingsBtn.textContent = 'Web \u9762\u677f\u8fde\u63a5';
    ...
    boot_on.addEventListener('click', async () => {
      ...
      try {
        ...
        const result = await runShellWithRoot(isBootUp ? removeBootLinesCmd() : addBootLinesCmd());
        if (!result.success) {
          createToast(`修改开机自启失败<br>${textToHtml(result.content || '')}`, 'red', 8000);
          return;
        }
        boot_on.style.background = isBootUp ? '' : 'var(--dark-btn-color-active)';
        createToast(isBootUp ? '\u5df2\u53d6\u6d88\u5f00\u673a\u81ea\u542f' : '\u5df2\u8bbe\u7f6e\u5f00\u673a\u81ea\u542f', 'green');
      } finally {
    ...
    clearCacheBtn.onclick = async () => {
      ...
      try {
        ...
        const runtimeLines = runtimeResults.map((item) => {
          ...
          return `${item.label}\uff1a\u5237\u65b0\u5931\u8d25\uff08HTTP ${item.statusCode || '\u65e0'}\uff09`;
        });
    ...
    const writeSubEntrypoint = async (
      ...
      const ok = await writeRuntimeConfigFromTemplate(cleanSources, { backup, showToast: false, forceTemplate: true });
      await appendTemplateFlowDebug(`leave writeSubEntrypoint ok=${ok ? '1' : '0'}`);
      return ok;
    };

    const persistSubSources = async (sources, mode = SUB_RULE_MODE_TEMPLATE) => {
      const cleanMode = normalizeSubRuleModeValue(mode);
      const storedSources = normalizeStoredSubSourceList(sources);
      if (storedSources.length == 0) return false;
      const subUrlsText = buildSubUrlsFileText(storedSources, cleanMode);
      const sourceRes = await runShellWithRoot(`
        mkdir -p ${shellQuote(CLASH_PROXY_DIR)}
        ${subRuleModePersistSidecarsCmd(cleanMode)}
        SUB=${shellQuote(CLASH_SUB_URLS)}
        KANO_CONVERT_LINE="$(grep -m 1 '^# KANO_SUB_CONVERT_MODE=' "$SUB" 2>/dev/null | tr -d '\r')"
        printf '%s' ${shellQuote(subUrlsText)} > "$SUB"
        if [ -n "$KANO_CONVERT_LINE" ]; then
          { sed -n '1p' "$SUB"; printf '%s\\n' "$KANO_CONVERT_LINE"; sed -n '2,$p' "$SUB"; } > "$SUB.kano_merge.$$"
          mv -f "$SUB.kano_merge.$$" "$SUB"
        fi
        chmod 600 "$SUB"
        first_line="$(sed -n '1p' ${shellQuote(CLASH_SUB_URLS)} 2>/dev/null | tr -d '\r')"
        [ "$first_line" = ${shellQuote(`# KANO_SUB_RULE_MODE=${cleanMode}`)} ] || { echo "SUB_URLS_MODE_HEADER_WRITE_FAILED:$first_line"; exit 1; }
        `);
      return sourceRes.success;
    };
    ...
    const inspectSubscriptionRuntimeConfig = async (sources = [], mode = SUB_RULE_MODE_TEMPLATE) => {
      const cleanSources = normalizeSubSourceList(sources);
      const cleanMode = normalizeSubRuleModeValue(mode);
      const expectedUrlChecks = cleanSources.map((source) => `
          "$YQ" e -r '[(."proxy-providers" // {})[] | (.url // "")] | .[]' "$CONFIG" 2>/dev/null | grep -Fx ${shellQuote(source.url)} >/dev/null 2>&1 || missing_urls=$((missing_urls + 1))
        `).join('\n');
      const res = await runShellWithRoot(`
        set +e
        CONFIG=${shellQuote(CLASH_CONFIG)}
...
${expectedUrlChecks}
      ...
      if (providerUrlCount != cleanSources.length) issues.push(`有效订阅 URL 数量不匹配：${providerUrlCount}/${cleanSources.length}`);
      ...
      const title = !configValidationResult.ok
        ...
        : allOk
          ...
          : providerResult.success > 0
            ? '订阅更新部分成功'
            : '配置已应用，但节点来源更新失败';
      const color = !configValidationResult.ok || confirmedNoCache || noUsableProvider ? 'red' : allOk ? 'green' : 'yellow';
      ...
      const { el, close } = createFixedToast(
        ...
        `<div style="pointer-events:all;width:92vw;max-width:720px;overflow-wrap:anywhere;word-break:break-word;">
          ...
          <div style="margin-top:12px;text-align:right;display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;">
            ...
            ${failedProviders.length > 0 ? `<button style="font-size:.64rem;background:var(--dark-btn-color-active)" id="${retryId}">重新更新失败项</button>` : ''}
            <button style="font-size:.64rem" id="${closeId}">关闭</button>
          </div>
        </div>`,
      );
      ...
      if (retryBtn) {
        retryBtn.onclick = async () => {
          ...
          try {
            ...
            safeClose();
            await showSubscriptionUpdateSelfCheck(sources, mode, mergedResult);
          } finally {
    ...
    const showSubscriptionUpdateSelfCheck = async (
      ...
      const controllerText = controllerCheck.success
        ...
        : `\u63a7\u5236 API\uff1a\u4e0d\u53ef\u8bbf\u95ee\uff08HTTP ${controllerCheck.statusCode || '\u65e0'}\uff09`;
      const ok = configCheck.ok && controllerOk;
      ...
      const repairable = !ok && configCheck.status == 'ok' && (
        configCheck.missingUrls > 0 ||
        configCheck.providerCount != expectedSourceCount ||
        configCheck.providerUrlCount != expectedSourceCount ||
    ...
    const updateSubProviders = async (sources, mode = SUB_RULE_MODE_TEMPLATE) => {
      const cleanMode = normalizeSubRuleModeValue(mode);
      const cleanSources = normalizeSubSourceList(sources);
      await appendTemplateFlowDebug(`enter updateSubProviders mode=${cleanMode} sources=${cleanSources.length}`);
      ...
      const runtimeCheck = await inspectSubscriptionRuntimeConfig(cleanSources, cleanMode);
      if (runtimeCheck.ok) {
        await appendTemplateFlowDebug('updateSubProviders fast path: runtime config unchanged');
        createToast('\u8fd0\u884c\u914d\u7f6e\u672a\u53d8\uff0c\u6b63\u5728\u76f4\u63a5\u5237\u65b0\u8282\u70b9\u8ba2\u9605...', 'yellow');
        const providerUpdate = await forceUpdateProvidersFromConfig({ showToast: false });
        await showSubscriptionUpdateSelfCheck(cleanSources, cleanMode, providerUpdate, runtimeCheck);
      ...
      if (!(await writeSubConfigByMode(cleanSources, cleanMode, { backup: true }))) {
      ...
      if (restarted) {
        let providerUpdate = buildProviderUpdateResult([]);
        if (cleanMode == SUB_RULE_MODE_TEMPLATE) {
    ...
    const writeSubConfigByMode = async (
      ...
      const cleanMode = normalizeSubRuleModeValue(mode);
      const cleanSources = normalizeSubSourceList(sources);
      await appendTemplateFlowDebug(`enter writeSubConfigByMode mode=${cleanMode} sources=${cleanSources.length}`);
      ...
      const ok = await writeSubEntrypoint(cleanSources, { backup });
      await appendTemplateFlowDebug(`leave writeSubConfigByMode template ok=${ok ? '1' : '0'}`);
      return ok;
    };

    const saveSubSources = async (sources, mode = SUB_RULE_MODE_TEMPLATE) => {
      const cleanMode = normalizeSubRuleModeValue(mode);
      const storedSources = normalizeStoredSubSourceList(sources);
      const cleanSources = normalizeSubSourceList(storedSources);
      await appendTemplateFlowDebug(`enter saveSubSources mode=${cleanMode} sources=${cleanSources.length} stored=${storedSources.length}`);
      ...
      if (!(await persistSubSources(storedSources, cleanMode))) {
    ...
    const overwriteConfigByTemplate = async ({ confirm = true } = {}) => {
      ...
      await appendTemplateFlowDebug(`enter overwriteConfigByTemplate mode=${currentMode} sources=${sources.length}`);
      ...
      if (sourceCheck.source == 'saved_subscription') {
        ...
        if (!(await writeSubEntrypoint(sources, { backup: true }))) {
        ...
        if (restarted) {
          const providerUpdate = await forceUpdateProvidersFromConfig({ showToast: false });
          await showSubscriptionUpdateSelfCheck(sources, currentMode, providerUpdate);
        }
        return restarted;
      }
      ...
      if (restarted) {
        const providerUpdate = await forceUpdateProvidersFromConfig({ showToast: false });
        await showSubscriptionUpdateSelfCheck([], currentMode, providerUpdate);
      }
      return restarted;
    };
    ...
    const writeEditableLocalFile = async (fileInfo, text) => {
      ...
      if (allowedFile.path == CLASH_SUB_URLS) {
        ...
        return await saveSubSources(sources, SUB_RULE_MODE_TEMPLATE);
      }
    ...
    const importSub = async () => {
      const currentSources = await readCurrentSubSources({ includeDisabled: true });
      const { el, close } = createFixedToast(
        'mm_sub_input_toast',
        `
            <div style="pointer-events:all;width:80vw;max-width:800px;">
      ...
      submitBtn.onclick = async () => {
        ...
        try {
          const success = await saveSubSources(sources, SUB_RULE_MODE_TEMPLATE);
    ...
    updateSubBtn.onclick = async () => {
      ...
      try {
        ...
        await updateSubProviders(sources, ruleMode);
      } finally {
    ...
    quickRunBtn.textContent = '\u5b89\u88c5 / \u542f\u52a8\u6838\u5fc3';
