...
UFI 原生启动顺序是否保持不变；增强尾钩是否幂等并同步快照；故障转移能否恢复冷启动覆盖；
手动隔离重放时，一条插件指令 `exit` 或卡死之后后面的插件能否继续；多行写法
（if/heredoc/函数）会不会被切坏；安装/卸载是否逐行无损；尾钩标记损坏、CRLF、
缺 shebang、`/proc/uptime` 读不到等异常输入。
